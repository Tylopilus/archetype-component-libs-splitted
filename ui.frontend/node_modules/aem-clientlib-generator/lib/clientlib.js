 ...
 */
async function removeClientLib(item, options, done) {
...
function start(itemList, options, done) {
  ...
  const dirs = getDirectories(options.context);

  // const adjustedDirs = dirs.map(dir => `${dir}/clientlibs`)
  console.log('meeeh ', dirs)
  if (_.isFunction(options)) {
    done = options;
    options = {};
  }
  ...
  itemList.forEach(item => processItem(item, options, dirs, false));
  // dirs.forEach(dir => processItem(item, options, dirs, processItemDone));
  // async.eachSeries(itemList, function (item, processItemDone) {

  //   // dirs.forEach(dir => processItem(item, options, dir, processItemDone));
  // }, done);
}
...
function normalizeAssets(clientLibPath, assets) {
  ...
  list.forEach(function (asset) {
    ...
    asset.files.forEach(function (file) {
      ...
      // console.log('teeeeest ',fileItem.src);
      // return;
 ...
 */
async function processItem(item, options, dirs, processDone) {

  for( let dir of dirs) {
  
    if(dir.includes('clientlib-site')) {
      continue;
    }
    if (!item.path) {
    ...
    // copy hack. Need to create a deep copy to not alter child objects
    const localItem = JSON.parse(JSON.stringify(item));
    // console.log(item)
  
    localItem.name=dir
    localItem.categories=[options.workspace+'.'+dir.split('/')[0]]
    localItem.assets.js.cwd = dir;
    localItem.assets.css.cwd = dir;
    console.log(localItem);
    // return;
  
    options.verbose && console.log("\n\nprocessing clientlib: " + localItem.name);
  
    // console.log(dir);
    // console.log(localItem.assets.js)
    const deleted = await removeClientLib(localItem, options);
    console.log(deleted)
    // return;
    if(deleted){
      var clientLibPath = localItem.outputPath || path.join(localItem.path, localItem.name);
  
      // create clientlib directory
      fse.mkdirsSync(clientLibPath);
  
      var serializationFormat = localItem.serializationFormat || SERIALIZATION_FORMAT_JSON;
      ...
      if (serializationFormat === SERIALIZATION_FORMAT_JSON) {
          ...
          writeClientLibJson(localItem, options);
      } else if (serializationFormat === SERIALIZATION_FORMAT_SLING_XML) {
          // write Sling-Initial-Content configuration
          writeClientLibSlingXml(localItem, options);
      } else {
          // write FileVault XML configuration
          writeClientLibXml(localItem, options);
      }
  

    //  console.log(localItem.assets); 
      var assetList = normalizeAssets(clientLibPath, localItem.assets);
      // const assetList=[];
      // console.log(assetList);
      // console.log(normalizeAssets(clientLibPath, item.assets));
      // return;
      // continue;
      ...
      assetList.forEach(asset => {
        ...
        if (asset.type === "js" || asset.type === "css") {
          options.verbose && console.log("clientLibPath:", clientLibPath);
          writeAssetTxt(clientLibPath, asset, options);
        }

        asset.files.forEach(fileItem => {
          ...
          if (fs.lstatSync(fileItem.src).isDirectory()) {
            fs.mkdirSync(fileItem.dest, { recursive: true });
          } else {
            fse.copySync(fileItem.src, fileItem.dest);
          }
        })
      })
    ...
    removeClientLib(localItem, function (err) {
      ...
      // console.log(assetList);
      // console.log(normalizeAssets(clientLibPath, item.assets.files));
      // return;
      // iterate through assets
      async.eachSeries(assetList, function (asset, assetDone) {
        // console.log("aaaaaaaaaaaaaaaa", asset);
